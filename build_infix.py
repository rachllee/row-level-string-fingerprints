...
Default mode: two separate columns (q8_prefix, q8_suffix) for two-column BETWEEN filtering.
Combined mode (--combined): one q16_infix column packing (prefix_code << suffix_bits | suffix_code)
  into a single UINT16, queried with equality or IN for multi-span patterns.
"""
...
def main():
    ...
    parser.add_argument("--combined", action="store_true",
                        help="Pack prefix+suffix into one q16_infix UINT16 column instead of two separate columns")
    ...
    if args.combined and prefix_bits + suffix_bits > 16:
        raise ValueError("--combined requires prefix_bits + suffix_bits <= 16")
    ...
    if args.combined:
        out_parquet = f"title_strs_infix16_p{prefix_bits}_s{suffix_bits}.parquet"
        combined_col = "q16_infix"
        print(f"Mode: COMBINED -> single {combined_col} UINT16 column")
    else:
        ...
        print(f"Mode: TWO-COLUMN -> {prefix_col} + {suffix_col}")
    ...
    n = len(df)
    print(f"Rows: {n:,}")
    print(f"Prefix: {prefix_bits} bits -> {prefix_buckets} buckets")
    print(f"Suffix: {suffix_bits} bits -> {suffix_buckets} buckets")
    ...
    prefix_codes = assign_codes(make_keys(df[COL], PREFIX_BYTES, suffix=False), prefix_boundaries, prefix_bits)
    ...
    suffix_codes = assign_codes(make_keys(df[COL], PREFIX_BYTES, suffix=True), suffix_boundaries, suffix_bits)
    ...
    if args.combined:
        # Pack (prefix_code << suffix_bits) | suffix_code into a single UINT16
        df[combined_col] = (
            (prefix_codes.astype(np.uint16) << np.uint16(suffix_bits)) | suffix_codes.astype(np.uint16)
        )
        df = df[[COL, combined_col]]

        df.to_parquet(out_parquet, index=False)
        print(f"\nWrote: {out_parquet}")

        vc = df[combined_col].value_counts()
        total_combined_buckets = prefix_buckets * suffix_buckets
        print(f"\n{combined_col} distribution:")
        print(f"  total possible buckets: {total_combined_buckets:,}")
        print(f"  non-empty buckets: {len(vc):,} / {total_combined_buckets:,}")
        print(f"  max bucket size:   {vc.max():,}")
        print(f"  min bucket size:   {vc.min():,}")
        print(f"  mean bucket size:  {vc.mean():.1f}")

        print("\nSample rows:")
        sample_df = df.sample(n=min(10, len(df)), random_state=42)
        print(sample_df.to_string())
    else:
        df[prefix_col] = prefix_codes
        df[suffix_col] = suffix_codes
